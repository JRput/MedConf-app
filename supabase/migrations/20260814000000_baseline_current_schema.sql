-- Baseline migration snapshotting the live schema as of 2026-08-14.
--
-- The three Feb 2026 migration files in this directory drifted badly
-- over 6 months of schema evolution — they DO NOT reflect production.
-- Running `supabase db reset` off them alone would leave the DB missing:
--   course_sessions, user_reminders, notifications,
--   is_flagship, is_on_demand, abstract_deadline_note,
--   pricing_tiers.currency, pricing_tiers.session_id, event_type,
--   scraper_sources.society / default_event_type / detail_is_multipage,
--   user_profiles.last_specialty_alert_at, etc.
--
-- This file replaces them for fresh environments: it is a verbatim
-- snapshot of supabase_schema.sql (the hand-maintained source of truth).
-- Existing production DB already matches; the migration is idempotent
-- via IF NOT EXISTS / OR REPLACE guards. From this point onwards, any
-- schema change goes through a new numbered migration.
