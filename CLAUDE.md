...
> **Production state (2026-06-26):** **12 active sources** — RCGP (1), RCSEng events (2), RSM (3), RCP (4), RCSEng courses (5), RCEM × 3 (6/7/8), RCOG × 2 (9/10), RCR × 2 (11/12). Daily scrape 02:00 UTC GitHub Actions matrix, 12 parallel workers. Conferences now have: `is_flagship` (international/national flagship split, surfaced as a directory sub-filter "International / National" vs "Regional / Local"), `is_on_demand` (RCEM catch-up recordings, displayed with "Available until X"), `abstract_deadline_note` (curator-confirmed open without published date), and `pricing_tiers.currency` (multi-currency — RCOG World Congress 2027 is in USD). Society chip on directory folds multi-source orgs (RCEM × 3, RCOG × 2, RCR × 2) into one chip. **NEW this session: autonomous remediator** at `medconf-scraper/remediator/` — patches scraped data autonomously via 7 fixers (description, venue, city, format, CPD, specialty, pricing, abstract_status) with tab-aware fetcher for Salesforce LWC pages (clicks every tab + snapshots body.innerText), vision LLM for image-based pricing (NVIDIA Llama 3.2 90B Vision via `vision.py`), per-field validators, and JSON reports under `reports/remediation/YYYY-MM-DD/`. CLI: `python -m remediator --source N`. Repo: https://github.com/JRput/MedConf-app · Latest commit: `65cf650` · **Active Supabase project:** `zcpszfbmvfylicpxgsfc` (eu-west-1, hotmail org). **For next-session handoff** see [memory/project_pending_work.md](file:///Users/Sushil/.claude/projects/-Users-Sushil-Documents-Documents-IMT2-Side-hustle-myTalk-conference-app/memory/project_pending_work.md).
